...
2. `mtrab/danfoss_ally` (custom_components/danfoss_ally/*.py) — HACS-интеграция,
   которую год использует пользователь.
...
| **140** | **"Inactive"→"active"** | **`output_status`** (термоактуатор) | **РЕШЕНО 2026-08-04**: пользователь поставил RT6 цель 32.5°C выше текущей, термоголовка в приложении показала "Открыто" — DP140 сменился ровно в этот момент с `"Inactive"` на `"active"` |
...
**РЕШЕНО (2026-08-04, живой тест на RT6):** пользователь переключил режим на
...
Пользователь изменил в приложении разом три уставки на RT6 (Дома 22.0→22.5,
В отсутствии 19.0→19.5, Пауза 5.0→5.5) и вернулся ко мне — пересняли живой
статус через `cid`, сравнили. Каждая уставка хранится **не в одном DP, а в
...
Живой тест (2026-08-04, все 6 RT разом): пользователь привязал термостаты к
комнатам, снял паузу, применил "Отдых → В отсутствии → Сейчас" с температурой
...
Pre-heat). Подтверждено живым тестом: пользователь выключил Pre-heat в HA →
...
аккаунта — один с Ally-радиаторами, один с Icon2 RT (как у пользователя). Их
